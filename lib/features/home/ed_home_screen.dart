...
/// ED Direct home screen — shows single ED status via direct connection.
class EdHomeScreen extends ConsumerWidget {
  const EdHomeScreen({super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(device?.name ?? 'End Device'),
        leading: IconButton(
        ...
        children: [
          ...
          Expanded(
            child: statusAsync.when(
              data: (status) => _buildEdStatus(context, status),
                ...
                ? Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'ALARM id=${evt.id} v0=${evt.v0} v1=${evt.v1} seq=${evt.seq}',
  ...
  Widget _buildEdStatus(BuildContext context, dynamic status) {
    return Padding(
        ...
        children: [
          Row(
            children: [
              ...
              Text(_profileName(status.profile),
                  style: Theme.of(context).textTheme.titleMedium),
              const Spacer(),
              Chip(label: Text('ED Direct')),
            ],
          ),
          ...
          Wrap(
            ...
            children: [
              ...
              MetricCard(label: 'TX Power', value: '${status.txPower}', unit: 'dBm'),
